...
<div id='crypto-donations'>
        ...
        <div class='crypto-donation'>
            <div class='crypto-donation-header'>
            <img src='https://s2.coinmarketcap.com/static/img/coins/64x64/74.png' style="width: 10vw; height: auto;">
                <h3>DOGE</h3>
            </div>
            <div class='crypto-donation-body'>
                <div class="crypto-donation-top">
                    <p class="thicc-text">Send DOGE to the following address:</p>
                    <div class='DOGE-code code-box'>
                        <p id='DOGE-address' class="wallet-address"></p>
                    </div>
                    <button class="copy-btn" onclick="DOGEcopyCode()">Copy</button>
                </div>
                <div class='crypto-donation-bottom'>
                    <p class="thicc-text">Or scan the following QR code:</p>    
                    <canvas id="DOGE-qr"></canvas>
                </div>
            </div>
        </div>

        <script type="text/javascript">
            var DOGEwalletAddress = "0x683e5376e3d0d4c9a465315ca564d8900c1d5562";
            new QRious({
                element: document.getElementById("DOGE-qr"), 
                value: DOGEwalletAddress,
                size: 400
            });
            function DOGEcopyCode() {
                var textToCopy = DOGEwalletAddress;
                navigator.clipboard.writeText(DOGEwalletAddress);
            }
            var DOGEwalletAddressElement = document.getElementById("DOGE-address");
            if (DOGEwalletAddress.length > 10) {
                if (DOGEwalletAddress.length >= 15) {
                    var fp = DOGEwalletAddress.substring(0, 10);
                    var lp = DOGEwalletAddress.substring(DOGEwalletAddress.length - 5);
                    var mp = ".....";
                    DOGEwalletAddressElement.textContent = fp + mp + lp;
                } else {
                    var shortenedText = DOGEwalletAddress.substring(0, 10) + ".....";
                    DOGEwalletAddressElement.textContent = shortenedText;
                }
            } else {
                DOGEwalletAddressElement.textContent = DOGEwalletAddress;
            }
        </script>
