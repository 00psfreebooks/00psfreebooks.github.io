...
old_base_url = "00psfreebooks.github.io"
new_base_url = "00psfreebooks.github.io"
your_domain = "https://" + new_base_url

old_gtag = "G-7QFRKMJ8TE"
new_gtag = "G-7QFRKMJ8TE"
...
# credentials are stored in environmental variable in codespaces, but in a file locally 
if not os.environ.get('CREDENTIALSJSON'):
    ...
    CREDENTIALS_JSON = os.environ.get('CREDENTIALSJSON')
...
def get_good_data():
    ...
    return all_folders, all_files, total_num_books

# Write code snippets and file information into files
def gen_site(folders2use, files2use, total_num_books):

    fancy_print("Creating ", book_index_file, 0)
    ...
    <p class='book_counter'>Total # Of Books: <i>{total_num_books}</i></p>""")
...
def generate_xml_sitemap():
    fancy_print("Creating ", sitemap_file, 0)

    # Delete the old sitemap.xml if it exists
    if os.path.exists(sitemap_file):
        os.remove(sitemap_file)

    # Walk through the ./docs/ directory and find HTML files
    html_files = []
    ...
    # Get book links from search index
    links = []

    with open(book_index_json_file, 'r', encoding='utf-8') as f:
        search_index = json.load(f)

    for category, books in search_index.items():
        for book in books:
            links.append(book[1])  # Assuming the link is the second item in each book list
    ...
    for html_file in html_files:
        ...
        loc_elem.text = f"{your_domain}/{html_file}"  # Creating absolute URLs
    ...
    tree.write(sitemap_file, encoding='utf-8')

def cleanup():

    fancy_print("Cleanup", "", 0)

    # Handle new Google Analytics Tag
        ...
        for file in files:
            if file.endswith('.html'):
                ...
                if old_gtag in content:
                    updated_content = content.replace(old_gtag, new_gtag)
                    with open(file_path, 'w') as f:
                        f.write(updated_content)
                    #print(f"Updated file: {file_path}")
    fancy_print("GTAG Updated: ", new_gtag, 1)
            ...
            if file.endswith('.html'):
                ...
                if old_base_url in content:
                    ...
                    with open(file_path, 'w') as f:
                        f.write(updated_content)
                    #print(f"Updated file: {file_path}")
    fancy_print("Base URL Updated To: ", new_base_url, 1)

# Execute Script
sys_checks()
folders2use, files2use, total_num_books = get_good_data()
misc_files()
gen_site(folders2use, files2use, total_num_books)
